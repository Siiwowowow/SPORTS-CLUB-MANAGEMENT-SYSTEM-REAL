...
const Signup = () => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      // Send user data to backend to add/update last_login
      const userInfo = {
        email: data.email,
        name: data.name,
        role: 'user',
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-100 p-4">
      <div className="card bg-white shadow-xl p-8 w-full max-w-md rounded-xl">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          {profile && <img src={profile} alt="Profile" className="w-16 h-16 rounded-full mx-auto" />}
          ...
          <button
            className="btn bg-[#EC7272] text-white w-full"
            disabled={uploading}
          >
